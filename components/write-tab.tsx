...
import { ArrowLeft, ArrowRight, FileText, Sparkles, Loader2, Save, RefreshCw } from "lucide-react"
...
export default function WriteTab() {
  ...
  const { chapters, generatedChapters, generateChapter, generateAllChapters, updateGeneratedChapter } =
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
          <div>
            ...
            <CardDescription>Generate and edit your story chapters</CardDescription>
          </div>
          <div className="flex gap-2">
            <Button
              onClick={handleGenerateAllChapters}
              disabled={isLoading("generateAllChapters") || chapters.length === 0}
              className="gap-2"
            >
        ...
        {chapters.length === 0 ? (
          <div className="text-center py-12 border rounded-md">
            ...
            <p className="text-muted-foreground max-w-md mx-auto mb-4">
              Please go back to the Outline tab and create your story structure first.
            </p>
            <Button variant="outline" onClick={() => document.getElementById("outline-tab-trigger")?.click()}>
              Go to Outline
            </Button>
          </div>
        ) : (
            ...
            <div className="md:col-span-1 space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {chapters.map((chapter, index) => {
                    const isGenerated = generatedChapters.some((c) => c.chapterId === chapter.id)
                    const isSelected = selectedChapterId === chapter.id
                    const isGenerating = isLoading(`generateChapter-${chapter.id}`)

                    return (
                      <Button
                        key={chapter.id}
                        variant={isSelected ? "default" : isGenerated ? "outline" : "ghost"}
                      ...
                      >
                        <div className="flex items-center gap-2 w-full">
                          ...
                          <span className="text-sm truncate text-left flex-1">{chapter.title}</span>
                          {isGenerating ? (
              ...
              {selectedChapter ? (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {!generatedChapter ? (
                      <Button
                        ...
                        disabled={isLoading(`generateChapter-${selectedChapterId}`)}
                        className="gap-2"
                      >
                  ...
                  <div className="border rounded-md">
                    {!generatedChapter ? (
                      <div className="flex flex-col items-center justify-center p-8 text-center">
                        ...
                        <h3 className="text-lg font-medium mb-2">Chapter Not Generated Yet</h3>
                        <p className="text-muted-foreground max-w-md mx-auto mb-4">
                      ...
                      <ScrollArea className="h-[500px]">
                        <div className="p-6 font-serif text-base whitespace-pre-line">{generatedChapter.content}</div>
                      </ScrollArea>
                    )}
                  </div>
                </div>
                ...
                <div className="flex flex-col items-center justify-center h-[500px] border rounded-md">
                  ...
                  <p className="text-muted-foreground max-w-md text-center">
                    Select a chapter from the list to view, generate, or edit its content.
                  </p>
                </div>
              )}
            </div>
      ...
      <CardFooter className="flex justify-between">
        <Button
          variant="outline"
          className="gap-2"
          onClick={() => document.getElementById("outline-tab-trigger")?.click()}
        >
          ...
          onClick={() => {
            if (generatedChapters.length === 0) {
              toast({
                ...
                description: "Please generate at least one chapter before proceeding.",
                variant: "destructive",
              })
              return
            }
